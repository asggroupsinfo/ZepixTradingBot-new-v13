...
class TelegramBot:
    ...
    def _show_logic_control_menu(self, user_id, message_id):
        ...
        keyboard = {
            "inline_keyboard": [
                [
                    {"text": "✅ Enable LOGIC1", "callback_data": "cmd_strategy_logic1_on"},
                    {"text": "⛔ Disable LOGIC1", "callback_data": "cmd_strategy_logic1_off"}
                ],
                [
                    {"text": "✅ Enable LOGIC2", "callback_data": "cmd_strategy_logic2_on"},
                    {"text": "⛔ Disable LOGIC2", "callback_data": "cmd_strategy_logic2_off"}
                ],
                [
                    {"text": "✅ Enable LOGIC3", "callback_data": "cmd_strategy_logic3_on"},
                    {"text": "⛔ Disable LOGIC3", "callback_data": "cmd_strategy_logic3_off"}
                ],
    ...
    def handle_set_profit_sl(self, message):
        ...
        try:
            ...
            if current_mode == 'SL-1.1':
                ...
                if not logic:
                    self.send_message("❌ SL-1.1 mode requires LOGIC parameter (LOGIC1, LOGIC2, LOGIC3)")
                    return
                
                logic = logic.upper()
                if logic not in ['LOGIC1', 'LOGIC2', 'LOGIC3']:
                    self.send_message("❌ Invalid logic. Use LOGIC1, LOGIC2, or LOGIC3")
                    return
    ...
    def handle_profit_sl_status(self, message):
        ...
        try:
            ...
            if mode == "SL-1.1":
                ...
                status_msg += f"  • LOGIC1: ${sl_1_1.get('LOGIC1', 20.0):.2f} SL\n"
                status_msg += f"  • LOGIC2: ${sl_1_1.get('LOGIC2', 40.0):.2f} SL\n"
                status_msg += f"  • LOGIC3: ${sl_1_1.get('LOGIC3', 50.0):.2f} SL\n"
            else:
        ...
        try:
            ...
            if isinstance(message, dict):
                ...
                if len(parts) >= 2:
                    ...
                    if parts[1].upper() in ["LOGIC1", "LOGIC2", "LOGIC3"]:
                        logic = parts[1].upper()
                        if len(parts) >= 3:
                            value = parts[2]
                    else:
            ...
            if calculator.current_mode == "SL-1.1":
                if not logic:
                     self.send_message("❌ Mode SL-1.1 requires specifying LOGIC (LOGIC1/LOGIC2/LOGIC3)")
                     return
    ...
    def handle_reset_profit_sl(self, message):
        ...
        try:
            ...
            calculator = self.profit_booking_manager.profit_sl_calculator
            calculator.sl_1_1_settings = {"LOGIC1": 20.0, "LOGIC2": 40.0, "LOGIC3": 50.0}
        ...
        try:
            ...
            if len(parts) < 3:
                self.send_message(
                    ...
                    "Example: /set_profit_sl LOGIC1 25\n"
                    "Valid logics: LOGIC1, LOGIC2, LOGIC3"
                )
                return
...
Use /dashboard to return to main view"""
    ...
    def _show_logic_control_menu(self, user_id: int, message_id: int):
        ...
        try:
            ...
            keyboard.append([])
            # Logic 1
            keyboard.append([{"text": "✅ Enable LOGIC1", "callback_data": "cmd_strategy_logic1_on"},
                            {"text": "❌ Disable LOGIC1", "callback_data": "cmd_strategy_logic1_off"}])
            # Logic 2
            keyboard.append([{"text": "✅ Enable LOGIC2", "callback_data": "cmd_strategy_logic2_on"},
                            {"text": "❌ Disable LOGIC2", "callback_data": "cmd_strategy_logic2_off"}])
            # Logic 3
            keyboard.append([{"text": "✅ Enable LOGIC3", "callback_data": "cmd_strategy_logic3_on"},
                            {"text": "❌ Disable LOGIC3", "callback_data": "cmd_strategy_logic3_off"}])
            keyboard.append([])
            # Navigation
            keyboard.append([{"text": "🔙 Back", "callback_data": "nav_back"},
